	...
	public MazeFrame(int rowNumber, int colNumber) {
		...
		button_4.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				maze.setBgmPause(false);
				if (maze.isGameOver()) {
					maze.creatAudioThread();
					maze.creatEnemyThread();
				}
				...
				int numofenemys = Integer.parseInt(spinner_3.getValue().toString());
				if (maze.getColNumber() == col && maze.getRowNumber() == row && maze.getNumofenemys() == numofenemys) {
					...
					maze.setNumofenemys(numofenemys);
					if (getCreateMaze() != maze.getCreateMaze()) {
						maze.setCreateMaze(getCreateMaze());
					}
					maze.createMaze();
		...
		lbNumOfEnemys = new JLabel("Num Of Enemys: ");
		panel_17.add(lbNumOfEnemys);
		
		spinner_3 = new JSpinner();
		spinner_3.setModel(new SpinnerNumberModel(1, 1, 20, 1));
		editor = new JSpinner.NumberEditor(spinner_3, "0");
		spinner_3.setEditor(editor);
		textField = ((JSpinner.NumberEditor) spinner_3.getEditor()).getTextField();
		textField.setEditable(false);
		panel_17.add(spinner_3);
		...
		button_1.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (!maze.isWin()) {
//					System.out.println(maze.isBgmPause());
					if (!maze.isBgmPause() && maze.isStartTiming() && button_1.isEnabled()) {
						button_1.setText("Continue");
						((Timers) maze.getTimeText()).stop();
						maze.setGameOver(true);
						maze.setBgmPause(true);
						maze.setPlayerClose();
					} else if (maze.isBgmPause() && maze.isStartTiming() && button_1.isEnabled()) {
						maze.setBgmPause(false);
						maze.creatEnemy();
						maze.creatAudioThread();
						maze.creatEnemyThread();
